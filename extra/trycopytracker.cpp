...
//------------------------------------------------g++ trycopyclient3.cpp sha1_impl.cpp -lssl -lcrypto----to compile
...
void* service_req(void	*arg)
{
	...
	cout<<req_clientport<<"\n";
	memset(&req_clientport,0,sizeof(req_clientport));
	recv(newsock,(void*)file_nm,200,0);
	cout<<file_nm;
	memset(&file_nm,0,sizeof(file_nm));
	 //recv(newsock,(void*)hashtot,strlen(hashtot),0);
	//cout<<hashtot;
	cout<<"\n";
	//memset(&hashtot,0,s(hashtot));
...
int main(int argc, char* argv[])
{
    ...
    // { 	
	    ...
	    while( (newsock = accept(socketfd,(struct sockaddr*) &newaddrs, (socklen_t*) &addrlen)) ) //accept is a blckng call
	    {
		    if(pthread_create(&tid, NULL, service_req, &newsock)!= 0 )
		    {
